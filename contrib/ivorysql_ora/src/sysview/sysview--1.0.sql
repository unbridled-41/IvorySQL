...
-- Oracle-compatible object catalog views.  OBJECT_TYPE is derived from the
-- underlying catalog: relations (TABLE/VIEW/MATERIALIZED VIEW/INDEX/
-- SEQUENCE/FOREIGN TABLE), routines (FUNCTION/PROCEDURE/AGGREGATE), types
-- (TYPE for composite and domain types), packages (PACKAGE) and triggers
-- (TRIGGER).  Packages keep spec and body in one catalog row, so a package
-- produces a single PACKAGE row.  PostgreSQL does not track DDL times, so
-- CREATED/LAST_DDL_TIME/TIMESTAMP are NULL; STATUS is always VALID.
-- Built-in scalar types of the sys/pg_catalog namespaces are not listed,
-- matching Oracle, which does not expose built-in types as objects either.

CREATE OR REPLACE VIEW SYS.DBA_OBJECTS AS
	SELECT
		SYS.ORA_CASE_TRANS(PG_GET_USERBYID(C.RELOWNER)::VARCHAR2)::VARCHAR2(128) AS OWNER,
		SYS.ORA_CASE_TRANS(C.RELNAME::VARCHAR2)::VARCHAR2(128) AS OBJECT_NAME,
		NULL::VARCHAR2(128) AS SUBOBJECT_NAME,
		C.OID::VARCHAR2(10)::NUMBER AS OBJECT_ID,
		...
		END::VARCHAR2(23) AS OBJECT_TYPE,
		...
		'N'::VARCHAR2(1) AS GENERATED,
		'N'::VARCHAR2(1) AS SECONDARY,
		C.RELNAMESPACE::VARCHAR2(10)::NUMBER AS NAMESPACE,
...
UNION ALL
	SELECT
		SYS.ORA_CASE_TRANS(PG_GET_USERBYID(P.PROOWNER)::VARCHAR2)::VARCHAR2(128) AS OWNER,
		SYS.ORA_CASE_TRANS(P.PRONAME::VARCHAR2)::VARCHAR2(128) AS OBJECT_NAME,
		NULL::VARCHAR2(128) AS SUBOBJECT_NAME,
		P.OID::VARCHAR2(10)::NUMBER AS OBJECT_ID,
		NULL::NUMBER AS DATA_OBJECT_ID,
		...
		'N'::VARCHAR2(1) AS GENERATED,
		'N'::VARCHAR2(1) AS SECONDARY,
		P.PRONAMESPACE::VARCHAR2(10)::NUMBER AS NAMESPACE,
		NULL::VARCHAR2(128) AS EDITION_NAME
	FROM PG_PROC AS P
	LEFT JOIN PG_TRIGGER AS T
		ON P.OID = T.TGFOID
...
UNION ALL
	SELECT
		SYS.ORA_CASE_TRANS(PG_GET_USERBYID(T.TYPOWNER)::VARCHAR2)::VARCHAR2(128) AS OWNER,
		SYS.ORA_CASE_TRANS(T.TYPNAME::VARCHAR2)::VARCHAR2(128) AS OBJECT_NAME,
		NULL::VARCHAR2(128) AS SUBOBJECT_NAME,
		T.OID::VARCHAR2(10)::NUMBER AS OBJECT_ID,
		NULL::NUMBER AS DATA_OBJECT_ID,
		...
		'N'::VARCHAR2(1) AS GENERATED,
		'N'::VARCHAR2(1) AS SECONDARY,
		T.TYPNAMESPACE::VARCHAR2(10)::NUMBER AS NAMESPACE,
		NULL::VARCHAR2(128) AS EDITION_NAME
	FROM PG_TYPE AS T
	WHERE T.TYPTYPE IN ('c', 'd')
		AND NOT EXISTS (SELECT 1 FROM PG_TYPE EL WHERE EL.TYPARRAY = T.OID)
		AND NOT EXISTS (SELECT 1 FROM PG_CLASS CR WHERE CR.RELTYPE = T.OID)
		AND T.TYPNAMESPACE != 'PG_CATALOG'::REGNAMESPACE::OID
		AND T.TYPNAMESPACE != 'PG_TOAST'::REGNAMESPACE::OID
		AND T.TYPNAMESPACE != 'INFORMATION_SCHEMA'::REGNAMESPACE::OID
		AND T.TYPNAMESPACE != 'SYS'::REGNAMESPACE::OID
UNION ALL
	SELECT
		SYS.ORA_CASE_TRANS(PG_GET_USERBYID(K.PKGOWNER)::VARCHAR2)::VARCHAR2(128) AS OWNER,
		SYS.ORA_CASE_TRANS(K.PKGNAME::VARCHAR2)::VARCHAR2(128) AS OBJECT_NAME,
		NULL::VARCHAR2(128) AS SUBOBJECT_NAME,
		K.OID::VARCHAR2(10)::NUMBER AS OBJECT_ID,
		NULL::NUMBER AS DATA_OBJECT_ID,
		'PACKAGE'::VARCHAR2(23) AS OBJECT_TYPE,
		NULL::TIMESTAMP AS CREATED,
		NULL::TIMESTAMP AS LAST_DDL_TIME,
		NULL::VARCHAR2(19) AS TIMESTAMP,
		'VALID'::VARCHAR2(7) AS STATUS,
		...
		'N'::VARCHAR2(1) AS GENERATED,
		'N'::VARCHAR2(1) AS SECONDARY,
		K.PKGNAMESPACE::VARCHAR2(10)::NUMBER AS NAMESPACE,
		NULL::VARCHAR2(128) AS EDITION_NAME
	FROM PG_PACKAGE AS K
	WHERE K.PKGNAMESPACE != 'PG_CATALOG'::REGNAMESPACE::OID
		AND K.PKGNAMESPACE != 'PG_TOAST'::REGNAMESPACE::OID
		AND K.PKGNAMESPACE != 'INFORMATION_SCHEMA'::REGNAMESPACE::OID
UNION ALL
	SELECT
		SYS.ORA_CASE_TRANS(PG_GET_USERBYID(CL.RELOWNER)::VARCHAR2)::VARCHAR2(128) AS OWNER,
		SYS.ORA_CASE_TRANS(T.TGNAME::VARCHAR2)::VARCHAR2(128) AS OBJECT_NAME,
		NULL::VARCHAR2(128) AS SUBOBJECT_NAME,
		T.OID::VARCHAR2(10)::NUMBER AS OBJECT_ID,
		NULL::NUMBER AS DATA_OBJECT_ID,
		...
		'N'::VARCHAR2(1) AS GENERATED,
		'N'::VARCHAR2(1) AS SECONDARY,
		CL.RELNAMESPACE::VARCHAR2(10)::NUMBER AS NAMESPACE,
		NULL::VARCHAR2(128) AS EDITION_NAME
	FROM PG_TRIGGER AS T
	JOIN PG_CLASS AS CL
		ON T.TGRELID = CL.OID
	WHERE CL.RELNAMESPACE != 'PG_CATALOG'::REGNAMESPACE::OID
		AND CL.RELNAMESPACE != 'PG_TOAST'::REGNAMESPACE::OID
		AND CL.RELNAMESPACE != 'INFORMATION_SCHEMA'::REGNAMESPACE::OID
ORDER BY
	OWNER, OBJECT_ID;
...
UNION ALL
	...
	WHERE K.PKGNAMESPACE != 'PG_CATALOG'::REGNAMESPACE::OID
		AND K.PKGNAMESPACE != 'PG_TOAST'::REGNAMESPACE::OID
		AND K.PKGNAMESPACE != 'INFORMATION_SCHEMA'::REGNAMESPACE::OID
		AND (PG_GET_USERBYID(K.PKGOWNER) = CURRENT_USER
			OR HAS_PACKAGE_PRIVILEGE(K.OID, 'EXECUTE'))
UNION ALL
	SELECT
		SYS.ORA_CASE_TRANS(PG_GET_USERBYID(CL.RELOWNER)::VARCHAR2)::VARCHAR2(128) AS OWNER,
		SYS.ORA_CASE_TRANS(T.TGNAME::VARCHAR2)::VARCHAR2(128) AS OBJECT_NAME,
	...
	WHERE CL.RELNAMESPACE != 'PG_CATALOG'::REGNAMESPACE::OID
		AND CL.RELNAMESPACE != 'PG_TOAST'::REGNAMESPACE::OID
		AND CL.RELNAMESPACE != 'INFORMATION_SCHEMA'::REGNAMESPACE::OID
		AND (PG_GET_USERBYID(CL.RELOWNER) = CURRENT_USER
			OR HAS_TABLE_PRIVILEGE(T.TGRELID,
				'SELECT,INSERT,UPDATE,DELETE,TRUNCATE,REFERENCES,TRIGGER'))
ORDER BY
	OWNER, OBJECT_ID;
...
CREATE OR REPLACE VIEW SYS.USER_CONS_COLUMNS AS
	SELECT
		...
		ARRAY_POSITION(CON.CONKEY, ATT.ATTNUM)::NUMBER AS POSITION
	FROM
